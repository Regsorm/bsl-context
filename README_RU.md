...
| `unknown_directive` | high / low | Имя директивы (`&НаСервере`, `&Перед`, …) не входит в whitelist (только в `validate_module`) |

high-confidence находки имеют false-positive близкий к нулю; low — зависят от
точности вывода типов и полноты `hbk`.

**Валидация целого модуля** (`validate_module`) — принимает ЦЕЛЫЙ текст модуля
(общий модуль, модуль объекта, модуль формы). Сервер через
`tree-sitter-onescript` сам извлекает объявления `Процедура`/`Функция` и не
считает их вызовы опечатками платформенных методов. Дополнительно проверяет
имена директив компилятора и расширений (`&НаСервере`, `&Перед("Foo")`, …).
Уровни/профили и формат ответа — те же, что у `validate_expression`.
...
| `validate_module` | Валидация целого BSL-модуля (с whitelist своих процедур и проверкой директив) |
