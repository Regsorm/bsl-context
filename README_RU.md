...
и после каждой доработки конфигурации. Пути он берёт из конфига, параметров не
принимает. Пока идёт сборка, валидация продолжает работать по прежней базе; если
сборка упадёт, прежняя база останется целой. То же самое можно сделать из командной
строки: `bsl-lite-index build --root <выгрузка> --db <файл.db>` (каталог должен
