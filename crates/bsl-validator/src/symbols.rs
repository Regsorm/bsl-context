...
pub trait SymbolSource: Send + Sync {
    ...
    /// Умеют все три источника (`lite`, `code_index_db`, `code_index_mcp`):
    /// признак «Глобальный» читается из XML общего модуля. Дефолт `false` —
    /// для источника, который такого признака не знает.
